...
Inductive ceval : com -> state -> result -> Prop :=
  ...
  | E_NonDetChoiceFirst : forall st c1 c2 r,
      st =[ c1 ]=> r ->
      st =[ c1 !! c2 ]=> r
  | E_NonDetChoiceSecond : forall st c1 c2 r,
      st =[ c2 ]=> r ->
      st =[ c1 !! c2 ]=> r
...
(**
  This example demonstrates that a triple where we assume a false condition is valid, as stated in the
  theorem assume_false.
*)
...
Proof.
  eapply hoare_seq.
  - apply hoare_asgn.
  - unfold hoare_triple. intros. inversion H. inversion H2. simpl in H0. rewrite H0 in H6. discriminate H6.
Qed.
...
Inductive cstep : (com * result)  -> (com * result) -> Prop :=
  ...
  | CS_Assume : forall st,
      <{ assume true }> / st --> <{ skip }> / st
  | CS_NonDetChoice1 : forall st c1 c2,
      <{ c1 !! c2 }> / st --> <{ c1 !! skip }> / st
  | CS_NonDetChoice2 : forall st c1 c2,
      <{ c1 !! c2 }> / st --> <{ skip !! c2 }> / st
  | CS_NonDetChoiceStep1 : forall st st' c1 c1',
      c1 / st --> c1' / st' ->
      <{ c1 !! skip }> / st --> <{ c1' !! skip }> / st'
  | CS_NonDetChoiceStep2 : forall st st' c2 c2',
      c2 / st --> c2' / st' ->
      <{ skip !! c2 }> / st --> <{ skip !! c2' }> / st'
  | CS_NonDetChoiceFinish : forall st,
...
Proof.
  ...
  eapply multi_step. apply CS_SeqStep. apply CS_Assume.
  eapply multi_step. apply CS_SeqFinish.

  (* ((X := X + 1) !! (X := 3)) *)
  eapply multi_step. apply CS_SeqStep. apply CS_NonDetChoice1.
  eapply multi_step. apply CS_SeqStep. apply CS_NonDetChoiceStep1. apply CS_AssStep. apply AS_Plus1. apply AS_Id.
  eapply multi_step. apply CS_SeqStep. apply CS_NonDetChoiceStep1. apply CS_AssStep. apply AS_Plus. simpl.
  eapply multi_step. apply CS_SeqStep. apply CS_NonDetChoiceStep1. apply CS_Asgn.
  eapply multi_step. apply CS_SeqStep. apply CS_NonDetChoiceFinish.
  eapply multi_step. apply CS_SeqFinish.
...
Proof.
  ...
  - inversion H; subst.
    + simpl. rewrite (IHa1 a1').
      * reflexivity.
      * assumption.
    + simpl. rewrite (IHa2 a2').
      * reflexivity.
      * assumption.
    + reflexivity.
  - inversion H; subst.
...
Proof.
  ...
  - (* NonDetChoice *)
    intuition.
    apply IHd1 in H0. clear IHd1.
    apply IHd2 in H. clear IHd2.
    eapply hoare_choice'.
      + eapply hoare_consequence; eauto.
      + eapply hoare_consequence; eauto.
Admitted.
